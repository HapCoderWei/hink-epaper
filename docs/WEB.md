...
3. 点击连接，选择 HINK（或旧缓存 ESL）；新站点来源需要重新授权，不能继承 localhost 授权。如果系统列表为空，点击“找不到？扫描全部 BLE 设备”，再选择 HINK、ESL 或可能显示为未知名称的价签。备用入口不按名称过滤，但仍只会尝试访问本项目的 GATT 服务，选错普通设备只会连接失败。
4. 选图后可反复点击“左转 90°”或“右转 90°”调整方向；换新图片时恢复原始方向。页面会先旋转原图，再进行铺满/留白和三色像素处理，因此预览与实际发送内容方向一致。
5. 选择裁切/留白及像素处理，确认预览，再发送。默认“自动选择”会判断缩放后的画面更接近 Logo/文字还是照片：Logo、文字和条码自动使用清晰硬阈值，避免抗锯齿边缘扩散成杂点；照片自动使用蛇形 Floyd–Steinberg 误差扩散，以黑白或红色像素密度模拟中间层次。也可以手动强制选择任一模式。测试图和全白按钮只在连接成功后可用。
