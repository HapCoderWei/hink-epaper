...
RAM uint8_t ble_name[] = {12, 0x09, 'H', 'I', 'N', 'K', '_', '0', '0', '0', '0', '0', '0'};
...
_attribute_ram_code_ void ble_disconnect_callback(uint8_t e, uint8_t *p, int n)
{
	...
	ota_started = 0;
	set_led_color(0);
	/* The ATC reference code did not restart connectable advertising here,
	 * which made the device disappear after the first client disconnected. */
	app_switch_to_indirect_adv(e, p, n);
}
...
_attribute_ram_code_ void ble_connect_callback(uint8_t e, uint8_t *p, int n)
{
	...
	/* Do not keep the common-anode status LED on for the whole connection. */
	set_led_color(0);
}
...
{
	bls_pm_setSuspendMask(HINK_DIAG_SLEEP_MASK);
}
...
void init_ble(void)
{
	...
	ble_name[7] = hex_ascii[mac_public[2] >> 4];
	ble_name[8] = hex_ascii[mac_public[2] & 0x0f];
	ble_name[9] = hex_ascii[mac_public[1] >> 4];
	...
	/* This project uses a 32 KB retention startup/linker layout.  The internal
	 * 32 kHz RC selected in main.c supplies the BLE wake timer. */
	bls_pm_setSuspendMask(HINK_DIAG_SLEEP_MASK);
