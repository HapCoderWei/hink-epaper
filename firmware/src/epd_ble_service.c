...
int epd_ble_handle_write(void *p)
{
	...
	case 0x00:
		...
		byte_pos = 0;
		/* Request a short interval for the following image upload. */
	...
	case 0x02:
		...
		if (byte_pos > sizeof(epd_buffer))
			byte_pos = sizeof(epd_buffer);
		return 0;
	// Write data to image buffer.
	case 0x03:
		if ((payload_len - 1) > (sizeof(epd_buffer) - byte_pos))
		{
			return 0;
		}
	...
	case 0x04: // decode & display a TIFF image
		...
		return 0;
	case 0x05: // Generate and display the known-good panel test pattern.
		epd_make_validation_pattern();
		byte_pos = sizeof(epd_buffer);
		ble_set_connection_speed(200);
