...
_attribute_ram_code_ void EPD_init(void)
{
    ...
    gpio_setup_up_down_resistor(EPD_CS, PM_PIN_UP_DOWN_FLOAT);

    gpio_write(EPD_CLK, 0);
    gpio_set_func(EPD_CLK, AS_GPIO);
    gpio_set_input_en(EPD_CLK, 0);
    gpio_set_output_en(EPD_CLK, 1);
    gpio_setup_up_down_resistor(EPD_CLK, PM_PIN_UP_DOWN_FLOAT);

    gpio_write(EPD_MOSI, 0);
    gpio_set_func(EPD_MOSI, AS_GPIO);
    gpio_set_input_en(EPD_MOSI, 0);
    gpio_set_output_en(EPD_MOSI, 1);
    gpio_setup_up_down_resistor(EPD_MOSI, PM_PIN_UP_DOWN_FLOAT);
}

_attribute_ram_code_ void EPD_idle_pins(void)
{
    /* This panel is still powered by VCC. Deselect SPI and do not assert
     * reset after DEEP_SLEEP: a reset transition wakes/resets the controller.
     * These levels are maintained by ordinary suspend, not deep retention.
     * Never drive BUSY, which is an OUTPUT from the panel. */
    gpio_write(EPD_CS, 1);
    gpio_write(EPD_RESET, 1);
    gpio_write(EPD_DC, 0);
    gpio_write(EPD_CLK, 0);
    gpio_write(EPD_MOSI, 0);
    gpio_set_input_en(EPD_BUSY, 0);
